...
namespace MinLib
{
	...
	void LanServer::GetIP(int64_t clientID, WCHAR* ipBuffer, int size)
	{
	...
	// 리턴 : 성공여부
	/*----------------*////////////////////////*----------------*/
	bool LanServer::Disconnect(int64_t clientID)
	{
		int index = FindIndex(clientID);
	...
	// 리턴 : (bool) 성공여부
	/*----------------*////////////////////////*----------------*/
	bool LanServer::SendPacket(int64_t clientID, StreamBuffer* packet)
	{
		int index = FindIndex(clientID);
		Session* session = &_sessionArray[index];
		bool ret = false;
	...
	bool LanServer::SendPost(Session* session)
	{
		...
		if (!InterlockedCompareExchange((LONG*)&session->sendFlag_, TRUE, FALSE))
		{
			...
			if (useSize == 0)
			{
				...
				session->sendFlag_ = FALSE;
				bool ret = true;
				if (session->sendQueue_.GetUseCount())
					ret = SendPost(session);
				return ret;
			}
	...
	// 리턴 : (int) 변환된 인덱스
	/*----------------*////////////////////////*----------------*/
	int LanServer::FindIndex(int64_t clientID)
	{
		int64_t mask = 0xffff;
